...
│  ├─ videos.js                   Video index (20 entries, 4 blocks)
...
- BAD EGG COUNT: the number of flagged Bad Eggs is hand-written, not live — 973 as of the Sept 17, 2026 dev stream, which called the review essentially complete, so it should move little from here (a few late flags can still land). If it moves, update together: the `what-is-a-bad-egg` answer in `faq.js` (then rebuild hubs + JSON-LD), the "973" callouts on `index.html` (timeline), `pre-registration.html` (§7) and `article-ronin-free-mint-guide.html`, the Bad Eggs section in `chatbot-official-posts.md` and the Sep 17 addendum in `chatbot-digest.md`, and the two search.js excerpts that quote it ("How many Bad Eggs are there?" and the dev-stream recap entry). The dated mentions inside `article-free-mint-complete.html` and `article-dev-stream-sept-17-recap.html` are tied to their dates and stay as published (the completion article's 365 was correct on completion day). The live count is the Status filter: marketplace.roninchain.com/collections/yakkamon?Status=Bad%20Egg.
